...
В рамках данного проекта проекта вам предстоит разработать учебную платформу для онлайн-курса по ORM и Hibernate.
...
Этот проект поможет вам закрепить навыки работы с JPA/Hibernate, Spring Boot и реляционными БД, а также понять типичные проблемы, с которыми можно столкнуться, работая с ORM (например, с проблемой ленивой загрузки).

Выполнив работу, вы получите практический опыт создания сложной серверной системы и добавите в портфолио полноценный проект.

## Что нужно сделать

Вам необходимо разработать веб-приложение (учебную платформу) на базе Spring Boot, которое использует Hibernate/JPA для доступа к базе данных PostgreSQL.
...
В результате вы разработаете гибкую модель данных с множеством связей и столкнётесь с практическими аспектами работы с ORM, научитесь реализовывать репозитории и CRUD-операции, а также писать интеграционные тесты для проверки работы приложения.
...
- **Приблизительные связи**:
  ...
  - Викторины: если решили, что тесты привязаны к курсу (например, финальный экзамен), то связь «один ко многим»: Course -> Quiz. Если тесты на модуль - тогда Course не напрямую связан с Quiz.
...
- **Приблизительные связи**:
  - «Многие к одному»: Submission -> Assignment (одно задание, много решений).
...
Структура базы данных, описанная выше, покрывает основные сущности. В вашем проекте могут появиться и другие (например, «Администратор» как отдельный класс пользователя, «Уведомление» и так далее), но следите, чтобы общее число не вышло за разумные пределы и все связи были обоснованными.

Вот приблизительная визуализация нашей учебной базы данных, созданная в Mermaid Live Editor:
...
Обратите внимание, что связи типа OneToMany и ManyToMany по умолчанию LAZY - то есть коллекции модулей в курсе, уроков в модуле, заданий в уроке, решений в задании, вопросов в квизе и так далее будут загружаться только при явном обращении. Это именно то, что нужно, чтобы в процессе работы обнаружить потенциальные места, где требуется либо включить fetch, либо реорганизовать логику доступа к данным. Связи ManyToOne и OneToOne по умолчанию EAGER (Hibernate специфика) - например, при загрузке Submission сразу загрузится объект User студента (если не указать LAZY вручную). В таких случаях можно явно поставить fetch = LAZY на @ManyToOne/@OneToOne, если хотите отложить загрузку (Hibernate это позволяет, хотя JPA стандарт для OneToOne EAGER). В проектах обычно стараются избегать лишнего EAGER, поэтому можете настроить все связи на LAZY, где это оправдано.
